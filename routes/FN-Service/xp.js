...
async function xp(fastify, options) {
    ...
    fastify.post('/fortnite/api/v1/profile/:accountId/xp/addSpecificAmount', async (request, reply) => {
        const { accountId } = request.params;
        const { amount, apiKey } = request.body;
        ...
        try {
            xp = xp + amount;
            profile.stats.attributes.xp += amount;
        } catch (err) {
            console.error(`Error In XP: ${err}`);
            xp = xp + 10;
        }
